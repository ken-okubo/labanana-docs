...
| `X-Overflow-Top/Right/Bottom/Left` | Overflow em pixels por direção |
| `X-Bleed-Tolerance` | Tolerância aceita em pixels (ex: `{"width": 4, "height": 4}`) |
...
:::warning `PUT /artworks/.../placements/{templateId}` NÃO dispara re-render
Atualizar o placement via `PUT` só atualiza os valores de posicionamento no banco. Para gerar a imagem com o novo posicionamento, chame **`POST /seller-products/me/{sellerProductId}/render`** novamente.
:::
